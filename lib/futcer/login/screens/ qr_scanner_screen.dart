...
class _QrScannerScreenState extends State<QrScannerScreen> with SingleTickerProviderStateMixin {
  ...
  bool _isReady = false; // ✅ مش رح يقرأ إلا بعد ما يصير ready
  ...
  void initState() {
    ...
    // ✅ أنيميشن للخط اللي بيتحرك داخل الإطار
    ...
    // ✅ تأخير ثانيتين قبل ما يبدأ يقرأ
    Future.delayed(const Duration(seconds: 2), () {
      if (mounted) setState(() => _isReady = true);
    });
  }

  // ✅ دالة التحقق من Base64 URL-safe
  ...
  Future<void> _handleBarcode(BarcodeCapture capture) async {
    ...
    // ✅ تحقق إن القيمة Base64 URL-safe فقط
    if (!_isBase64UrlSafe(code)) {
      debugPrint('❌ ليس Base64 URL-safe: $code');
      return; // يتجاهله ويكمل المسح
    }
